...
function PullLoad(config) {
    ...
    this.emptyStop = config.emptyStop;
    this.emptyDataCloseTime = config.emptyDataCloseTime;
    ...
    this.down = Object.assign({},{
        ...
        init:`<div class="pull-load-down"><span><span class="pull-load-down-icon"></span>${config.textConfig.upText || '下拉刷新...'}</span></div>`,
        loading:`<div class="pull-load-down pull-load-loading"><span class="pull-load-down-icon"></span>${config.textConfig.upLoadText || '刷新中...'}</span></div>`,
        release:`<div class="pull-load-down pull-load-flip"><span><span class="pull-load-down-icon"></span>${config.textConfig.upReleaseText || '释放刷新...'}</span></div>`,
        emptyData:`<div class="pull-load-empty-data">${config.textConfig.upDataText || '没有更多数据了'}</div>`,
    ...
    this.up = Object.assign({},{
        ...
        distance:50,
        container:'<div class="pull-load-container"></div>',
        init:`<div class="pull-load-up"><span><span class="pull-load-up-icon"></span>${config.textConfig.downText || '上拉加载更多...'}</span></div>`,
        loading:`<div class="pull-load-up pull-load-loading"><span class="pull-load-up-icon"></span>${config.textConfig.downLoadText || '加载中...'}</span></div>`,
        release:`<div class="pull-load-up pull-load-flip"><span><span class="pull-load-up-icon"></span>${config.textConfig.downReleaseText || '释放加载...'}</span></div>`,
        emptyData:`<div class="pull-load-empty-data">${config.textConfig.downDataText || '没有更多数据了'}</div>`,
        id:'_____pull_up____',
        enable:true,
